...
#The EQ-5D levels, countries and corresponding methods preloaded as a table for later use
scoreData = read.table(
  ...
  EQ-5D-3L	Spain	TTO
  EQ-5D-3L	Spain	VAS
  EQ-5D-3L	Sri_Lanka	TTO
  EQ-5D-3L	Sweden	TTO
  EQ-5D-3L	South_Korea	TTO
  EQ-5D-3L	Taiwan	TTO
...
server <- function(input, output, session) {
  ...
  output$tab_selector_multiple = renderUI({ #creates type of calculation
    tabsetPanel(type = "tabs",
                ...
                tabPanel("Info", verbatimTextOutput("summ_error")),
                tabPanel("Summary Table", tableOutput("table")),
                tabPanel("Plot", plotOutput("plot")),
                tabPanel("Data with index values", tableOutput("mod.data")))
  })
  ...
  calculate<-reactive({
    ...
    if(input$Score=="EQ-5D-5L"){
        if(input$Method=="CW"){
          ans<-eq5dmap5Lto3L(dataset,input$col1,input$col2,input$col3, input$col4,input$col5,input$Country,input$Method,input$gender,c(input$agerange[1],input$agerange[2]))
        }else{
          ans<-valueEQ5D5L(dataset,input$col1,input$col2,input$col3, input$col4,input$col5,input$Country,input$gender,c(input$agerange[1],input$agerange[2]))
        }
     }else{
        ans<-valueEQ5D3L(dataset,input$col1,input$col2,input$col3, input$col4,input$col5,input$Country,input$Method,input$gender,c(input$agerange[1],input$agerange[2]))
    }
    return(ans)
  })
  calculate_single<-reactive({
    ...
    if(input$Score=="EQ-5D-5L"){
       if(input$Method=="CW"){
          ans<-eq5dmap5Lto3LIndscores(input$Country,input$Method,input$col1,input$col2,input$col3, input$col4,input$col5)
       }else{
          ans<-valueEQ5D5LIndscores(input$Country,input$col1,input$col2,input$col3, input$col4,input$col5)
       }
    }else{
       ans<-valueEQ5D3LIndscores(input$Country,input$Method,input$col1,input$col2,input$col3, input$col4,input$col5)
    }
  })
  
  
  observeEvent(input$goButton, {
    output$summ_error <- renderText({
      paste0('You have selected : Score as ', input$Score, ' - for the country ', input$Country,' with gender ' , input$gender,
          ' and age between ', input$agerange[1], ' and ', input$agerange[2])
    ...
    output$summary <- renderText({
      answer<-calculate_single()
      paste0('You have selected : Score as ', input$col1,input$col2,input$col3, input$col4,input$col5,' Country as ', input$Country,
             ': The index value is ', answer)
    })
    ...
    output$download <- downloadHandler({
      filename = function() {
        file<-paste(input$Score, "_",input$Country,"_",input$Method,".csv", sep = "")
      }
      content = function(file) {
        dataset <- datasetInput()
        answer<-calculate()
        write.csv(answer$modifiedData, file, row.names = FALSE)
      }
    })
      
    output$plot <- renderPlot({
      answer<-calculate()
      plot(answer$histogram)
    })
