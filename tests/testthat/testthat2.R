...
test_that("EQ5D3L scoring", {
  ...
  total_entries=seq(1,nrow(answers))
  end1=length(total_entries)
  VAS_countrylist=c("Argentinna","Belgium","Denmark" ,"Europe","Finland","Germany","NewZealand","Slovenia","Spain","UK")
  TTO_countrylist=c("Argentinna","Brazil","Canada","Chile","Denmark","France","Germany","Italy","Japan","Korea",
  ...
  for(j in 1:total){
    ...
    if(all_countries[j]%in%common_countries){
      ...
      for (i in 1:nrow(answers)){
        ...
        expect_equal(the.result.TTO,this.col.TTO[total_entries[i]],tolerance=1e-3)
        expect_equal(the.result.VAS,this.col.VAS[total_entries[i]],tolerance=1e-3)
      }
    }else{
      ...
      for (i in 1:nrow(answers)){
        ...
        expect_equal(the.result,as.numeric(this.col[total_entries[i]]),tolerance=1e-3)
      }
    }
  }
})
#################################################################################################################
# context("testing EQ5D3L valuation")
# test_that("test for value3L", {
#   set.seed(17)
#   sampledata <- data.frame(age=abs(rnorm(10, 60, 20)),
#                            sex=factor(sample(c("M", "F"), 10, replace=T)),
...
#                            eq5d3L.q4=(sample(c(1,2,3), 10, replace=T)),
#                            eq5d3L.q5=(sample(c(1,2,3), 10, replace=T)))
#   answer=valueEQ5D3L(sampledata,"eq5d3L.q1","eq5d3L.q2","eq5d3L.q3","eq5d3L.q4","eq5d3L.q5","UK","TTO",NA,NULL)
# })
...
test_that("Crosswalk EQ5D5L to 3L values", {
  ...
  the.result<-eq5dmap5Lto3LIndscores("UK","CW",c(1,2,3,4,5))
  expect_equal(the.result,0.06333624,tolerance=1e-3)
  the.result<-eq5dmap5Lto3LIndscores("JP","CW",1,2,3,4,5)
  expect_equal(the.result,0.0633362,tolerance=1e-3)
  the.result<-eq5dmap5Lto3LIndscores("UK","CW",8,2,3,4,5)
  expect_equal(the.result,-1,tolerance=1e-3)
  the.result<-eq5dmap5Lto3LIndscores("UK","CW",77777,2,3,4,5)
  expect_equal(the.result,-1,tolerance=1e-3)
...
test_that("Crosswalk EQ5D5L to 3L values", {
  ...
  total_entries=seq(1,nrow(answers))
  correct_ones=total_entries
  end1=length(correct_ones)
  for (i in 1:nrow(answers)){
    the.result<-eq5dmap5Lto3LIndscores("UK","CW",answers$state[correct_ones[i]])
